...
| 前端 | dev（免编译） | 打包 |
|---|---|---|
| TUI | `scripts\1.bat tui` | （无独立包，含在 Python 项目） |
| CLI | `scripts\1.bat cli <子命令>` | （无独立包） |
| GUI | `scripts\1.bat gui`（pytauri-wheel，**免 Rust**） | `scripts\1.bat`（standalone，出 .msi/.exe） |

## 快速验证（不打包）
```bash
scripts\1.bat                 # 全量测试
scripts\1.bat tui              # 跑 TUI
scripts\1.bat cli list-saves   # 跑 CLI
scripts\1.bat gui              # 跑 GUI（vite HMR + pytauri-wheel，免 Rust）
```

## GUI 打包（standalone pytauri → .msi/.exe）
```bash
scripts\1.bat            # 产物在 build/bundle-release/
```
...
- 发版 `release.yml`，两种触发：
  ...
  - 流程：装 Rust+Node+uv → 测试 → build-gui.bat → 上传 .msi/.exe 到 Release。
...
4. **打包前杀残留 exe**：跑着的 sse-gui.exe 占着 pyembed 的 DLL，rebuild 报 os error 32。build-gui.bat 已自动 taskkill。
