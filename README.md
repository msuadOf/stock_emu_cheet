...
按作用域分组。原版功能标注「原」，社区贡献标 [Extra]。

| 功能 | 作用域 | TUI | CLI | GUI |
|------|--------|-----|-----|-----|
| 改 PE / PB / 负债率 | 单股 | ✓原 | ✓ | ✓ |
| 改价格 / 涨跌停 | 单股 | ✓原 | ✓ | ✓ |
| 改主力/散户挂单（中位/倍数/自定义） | 单股 | ✓原 | ✓ | ✓ |
| 自由设定全部财务字段（防回滚） | 单股 | ✓原 | ✓ | ✓ |
| 查看股票完整详情 | 单股 | ✓原 | ✓ show-detail | ✓ |
| 改玩家持仓（增/改/删+总资金） | 全局 | ✓原 | ✓ | ✓ |
| NPC 购买取向预设（1-5 模式） | 全局 | ✓原 | ✓ | ✓ |
| 清空公告 / 交易历史 / 裁剪 HuddleNpc | 全局 | ✓原 | ✓ | ✓ |
| 批量改持仓（含 6 扣仓位策略） | 多选 | ✓ | — | ✓ |
| 批量改挂单 / NPC 取向 | 多选 | — | — | ✓ |
| **按板块筛选 + 批量操作** | 板块 | — | list-by-sector | ✓（筛选+选该板块全部） |
| [Extra] 分红 / 定向增发 / 退市 | 单股 | ✓ | ✓ | ✓ |
| [Extra] 市场整顿 / 砍机构转散户 | 全局 | ✓ | ✓ | ✓ |
| [Extra] 发布公告 / 业绩报告 | 单股 | ✓ | ✓ publish-notice | ✓ |
| [Extra] 查看公告 / 业绩报告 | 单股 | ✓ | ✓ list-notices | ✓ |

> TUI 菜单项见下方「单只股票菜单」「主菜单」。CLI 用 `sse-cli <子命令> --help` 查参数。GUI 是图形界面，单股/批量/全局操作分 fieldset。

### GUI 批量操作（持仓）### GUI 批量操作（持仓）
...
**按板块批量**：GUI 股票列表有「板块筛选」下拉——选某板块后点「选该板块全部」即可把该板块所有股票加入多选，再走上面的批量操作（持仓/挂单/取向都支持）。CLI 用 `sse-cli list-by-sector <sector> --save x.sav` 列板块股票。
