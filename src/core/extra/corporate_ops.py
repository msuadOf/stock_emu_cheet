...
**已迁移到 SaveModel**：函数收 ``SaveModel``/``StockModel``/``InfoModel``。
分红/增发的筹码运算与守恒全程在**内部值**上精确整数运算（与存档约定、与原 TUI 一致）；
退市/构造新股等结构性改动直接走内部 dict 树。
...
def cash_dividend_limits(info, total_hand):
    ...
    info: InfoModel 或原始 info dict（读 AssetNet/AssetLoan 内部值）。
    ...
    返回 (max_total, max_D)。
    """
    d = info._d if hasattr(info, "_d") else info
    asset_net = d.get("AssetNet", 0)
    asset_loan = d.get("AssetLoan", 0)
...
# [extra]
def apply_cash_dividend(save, code, stock, vols, D_int):
    """执行现金分红：把 D 分发到所有持仓者、除息降股价、扣减净资产。

    save: SaveModel。stock: StockModel。
    - vols: {账户键: 持仓量(内部值)}（含 'player'/'inst'/'ret' + 5 类 NPC）
    ...
    for k, vol in vols.items():
        ...
        if add == 0:
            continue
        if k == "player":
            player_d = save.player._d
            player_d["Amount"] = int(player_d.get("Amount", 0)) + add
            player_d["AmountInit"] = int(player_d.get("AmountInit", 0)) + add
        elif k == "inst":
            inst_d = stock._d["Institution"][0]
            inst_d["AmountUsableBuy"] = int(inst_d.get("AmountUsableBuy", 0)) + add
        elif k == "ret":
            ret_d = stock._d["Retail"][0]
            ret_d["AmountUsableBuy"] = int(ret_d.get("AmountUsableBuy", 0)) + add
        else:
...
# ------------------------------------------------------------------
# [extra]
def apply_stock_dividend(save, code, stock, X):
    """执行送股(10送X)：按比例增加所有持仓、股价等比下降、总市值不变。

    save: SaveModel。stock: StockModel。返回 (new_flow, new_total, new_price)（内部值）。
    """
    info_d = stock.info._d
    ...
    inst_d["VolumeUsableSell"] = int(iv * r)
    inst_d["InitVolumeSell"] = inst_d["VolumeUsableSell"]
    # 散户
    ret_d = stock._d["Retail"][0]
    rv = int(ret_d.get("VolumeUsableSell", 0))
    ret_d["VolumeUsableSell"] = int(rv * r)
    # NPC
    market = save._d.get("Market", {})
    for k in NPC_KEYS:
        for acc in market.get(k, []) or []:
...
# [extra]
def apply_private_placement(save, code, stock, ns, cost, candles):
    ...
    """
    info_d = stock.info._d
    info_d["VolumeFlow"] = int(info_d.get("VolumeFlow", 0)) + ns
    info_d["VolumeTotal"] = int(info_d.get("VolumeTotal", 0)) + ns
    if "VolumeFlowInit" in info_d:
        info_d["VolumeFlowInit"] = info_d["VolumeFlow"]
    player = save.player._d
...
# ------------------------------------------------------------------
# [extra]
def remove_stock_from_market(save, code):
    """从股票池删除某 code；返回被删的 stock（或 None）。save: SaveModel。"""
    removed = None
    new_stocks = []
    for s in save._d["Market"]["Stocks"]:
        if s["Info"]["Code"] == code:
            removed = s
        else:
            new_stocks.append(s)
    save._d["Market"]["Stocks"] = new_stocks
    return removed


# [extra]
def remove_code_notices(save, code):
    """从 NoticeGroup 中删除某 code 的所有公告（兼容 dict/list 两种形态）。save: SaveModel。"""
    ng = save._d["Market"].get("NoticeGroup", {})
    if isinstance(ng, dict):
        for key in list(ng.keys()):
            ng[key] = [item for item in ng[key] if item.get("Code") != code]
    elif isinstance(ng, list):
        save._d["Market"]["NoticeGroup"] = [item for item in ng if item.get("Code") != code]


# [extra]
def remove_player_position(save, code):
    """删除玩家某 code 的持仓；返回 [(Amount, VolumeUsable)] 便于估算亏损。save: SaveModel。"""
    sp = save.player._d.get("StockPos", [])
    removed = [(p.get("Amount", 0), p.get("VolumeUsable", 0))
               for p in sp if p.get("Code") == code]
    save.player._d["StockPos"] = [p for p in sp if p.get("Code") != code]
    return removed


# [extra]
def delist_to_b(save, code):
    """完全退市(进 B 集合)：从股票池删除、清公告、删玩家持仓、A 移出/B 加入。

    save: SaveModel。返回 (removed_stock, player_positions)。
    """
    from .notice_ops import get_or_create_delisted_pool
    pool = get_or_create_delisted_pool(save)
    a_set = pool["A"]
    b_set = pool["B"]
    stock = remove_stock_from_market(save, code)
    remove_code_notices(save, code)
    positions = remove_player_position(save, code)
    if code in a_set:
        a_set.remove(code)
...
# [extra]
def delist_to_a(save, code, rate_limit=0.05):
    """警告退市(进 A 集合)：限制 RateLimit，加入 A 集合。save: SaveModel。返回是否成功。"""
    from .notice_ops import get_or_create_delisted_pool
    pool = get_or_create_delisted_pool(save)
    a_set = pool["A"]
    ...
    if stock is None:
        return False
    stock.info.rate_limit = rate_limit
...
def build_new_stock_custom(new_code, default_info, total_shares_internal, floats_internal,
    ...
    财务字段取自 default_info（内部值，原样透传）；股数参数为内部值。
    """
    return {
        "Info": {
            "Code": new_code,
...
# [extra]
def attach_stock_to_market(save, new_stock, raw_price, sector_num, bourse_num,
                           mode="restore", restore_code=None):
    """把构造好的新股挂接到市场：生成初始 K 线、加入股票池、挂接 Sectors、
    （恢复模式）从退市池 B 移除 code。save: SaveModel。返回 new_stock。

    初始 K 线 Volume/Amount 用内部值（VolumeFlow 内部值 /10000 得「手」量级）。
    """
    ...
    save._d["Market"]["Stocks"].append(new_stock)

    market = save._d["Market"]
